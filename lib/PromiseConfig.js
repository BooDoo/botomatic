...
PromiseConfig.promiseConfigs.promiseDefaults = {
    ...
    processBody:  function(body) {
                    if (typeof body !== 'object') {
                      try {
                        //util.debug('Parsing body to JSON object...');
                        body = JSON.parse(body);
                      }
                      catch(err) {
                        //util.debug("Tried to parse JSON, failed. Returning body unparsed");
                      }
                    }
                    return body;
                  },
  ...
  processBody:  function(body) {
                  if (typeof body !== 'object') {
                    try {
                      ...
                      util.debug("Tried to parse JSON, failed. Returning body");
                    }
                  }
...
PromiseConfig.promiseConfigs.wordnikPhrasePres = PromiseConfig({
  ...
  processBody:  function(res) {
                  res = JSON.parse(res);
                  //util.debug("Phrases returned: " + res.length);
                  var stripRE = /^(?:the|a|an|and|or|no[tn]?|your?|my|theirs?|ours?|his|her|in|on|very|mere|it|it'?s|this|quite|that|these|one|very|to|by|those|own|.+'(?:'s|re|d)||each|every|on|of|be|was|re|were)$/ig;
                  return _(res).pluck("gram1").unique()
                    .filter(function (gram, i, a) {
                      //gram.match(stripRE) ? util.debug("Found a gram to strip out: " + util.inspect(gram)) : null;
...
PromiseConfig.promiseConfigs.ohhlaList = PromiseConfig({
  ...
  processBody:  function(body) {
                  var relativePath = utils.randomFromArray(body.match(this.targetRE))
                                    .replace(this.stripRE,'')
                                    .trim ();
                  //util.debug('Returning http://ohhla.com/' + relativePath + ' from randomTopOhhla...')
...
PromiseConfig.promiseConfigs.cowboyIndex = PromiseConfig({
  ...
  processBody:  function(body) {
                  try {
                    ...
                    util.error("Can't parse site index: " + util.inspect(e));
                    return;
                  }
                }
});
