...
PromiseConfig.promiseConfigs.wordnikPartOfSpeech = PromiseConfig({
  ...
  processBody:  function(res) {
                  //console.log("wordnikPOS processBody RES:",res);
                  //console.log("typeof res:", typeof res);
                  res = JSON.parse(res);
                  return _(res).pluck("partOfSpeech").unique().value();
                },
  validate:     function(error, response, body) {
...
//TODO: Use me!
PromiseConfig.promiseConfigs.wordnikPhraseDescs = PromiseConfig({
  handle:       "wordnikPhraseDescs",
  processBody:  function(res) {
                  //console.log("wordnikPOS processBody RES:",res);
                  //console.log("typeof res:", typeof res);
                  res = JSON.parse(res);
                  var stripRE = /^(?:the|a|an|and|or|no[tn]?|your?|my|theirs?|ours?|his|her|this|that|these|those|own|.+'(?:s|re|d)||each|every|on|of|be|was|re|were)$/ig;
                  return _(res).pluck("gram1").unique()
                    .filter(function (gram, i, a) {
                      return !stripRE.test(gram)
                    })
