...
function Bot (botConfig, botStore) {
  ...
  if (this.type === 'lyrpictweet') {
    ...
    this.populateHowilikeit(this); //Initial population of the random word pools.
...
Bot.prototype.makeSnowclone = function(bot) {
  ...
  for (w in words) {
    //console.log("Selecting a word from",w+"s...");
    wordPool = bot[w + 's'];
...
Bot.prototype.populateHowilikeit = function(bot) {
  ...
  //Always aim for 5 objects available with 3 or more valid descriptors
  ...
  .then(
    function(rws) {
      ...
      _.each(rws, function (rw) {
        //console.log("Making descriptors promise for",rw);
        when(Word.descriptors(rw))
        .then(
          function(descs) {
            //console.log("descs for",rw,":",descs);
            descs = _.without(descs, rw);
            if (descs.length < 3) {
              //console.log("not enough descriptors for",rw);
              //delete objects[rw];
            }
            else {
              //console.log(rw,"looks good");
              objects[rw] = rw;
            }
          },
...
Bot.prototype.makeHowilikeit = function(bot) {
  ...
  //console.log("bot.objects:",bot.objects);
  if (!bot.objects || _.isEmpty(bot.objects)) {return;} //No objects? Bail.
