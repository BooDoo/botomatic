var Twit        = require('twit'),
    _           = require('lodash'),
    Word        = require(__dirname + '/Word.js'),
    utils       = require(__dirname + '/utils.js'),
    Promises    = require(__dirname + '/Promises.js'),
    util        = require('util'),
    when        = require('when'),
...
function Bot (botConfig, botStore) {
  ...
  else if (this.type === 'combinator') { //HACKY! TODO: Abstract out
    this.descriptors = this.descriptors || [];
    this.gameTypes = this.gameTypes || [];
    this.endings = this.endings || [];
    this.platforms = this.platforms || [];
    //this.searchIntervalId = setTimeout(this.populateLatourAndOrderSources, this.searchInterval, this);
    this.intervalId = setTimeout(this.w8edgame, this.interval, this);
  }

  Bot.bots[this.handle] = this;
}

Bot.prototype.w8edgame = function w8edgame(bot) {
	...
	desc1 = utils.randomFromArray(bot.descriptors);
	desc2 = utils.randomFromArray(bot.descriptors);
	gameType = utils.randomFromArray(bot.gameTypes);

	switch (_.random(2)) {
          case 0:
		description = [desc1,gameType].join(' ');
          break;
          case 1:
            description = [desc2,gameType].join(' ');
          break;
          case 2:
            description = [desc1,desc2,gameType].join(' ');
          break;
	}

	platform = utils.randomFromArray(bot.platforms);
	ending = utils.randomFromArray(bot.endings);

	tweetContent = bot.template({description: description, platform: platform, ending: ending});

	if (tweetContent.length > 140) {
        //try again, kid
          return bot.w8edgame(bot);
	}
	else {
          utils.postTweet(bot.T, tweetContent);
          return setTimeout(bot.w8edgame, bot.interval, bot);
	}
}
...
Bot.prototype.populateRandomWords = function(bot) {
  ...
  _.each(words, function(reqParams, wh) {
    ...
    .then(function(list) {
      //util.debug("Populating bot." + wh +"s with: " + list);
    ...
    function(rws) {
      _.each(rws, function (rw) {
        ...
        .then(
          ...
          function(e) {util.error("ERR " + util.inspect(e));}
        )
      })
    },
    function(e) {util.error("ERR for " + word + ": " + util.inspect(e));}
  );
};
...
Bot.prototype.populateIMDBDescriptions = function populateIMDBDescriptions(showId, seasonCount, customReplace, targetPool, bot) {
  ...
  function (err) {
      console.err("There seems to be a problem...\n", err);
  });
...
Bot.prototype.composeTweetMash = function composeTweetMash (bot) {
  ...
  // util.debug("of " + firsts.length + " firsts, selected: " + first);
  second = utils.randomFromArray(seconds, false, null, true); //seconds[Math.floor(Math.random() * seconds.length)];
  // util.debug("of " + seconds.length + " seconds, selected: " + second);
  ...
  //util.debug(tweetContent);
  //util.debug(tweetContent.length);
...
Bot.prototype.flickrRandomPhotoURL = function flickrRandomPhotoURL(bot) {
  ...
  return Promises.makeFlickrPromise(bot.getFlickrURL(bot), bot).then(function(flickrResponse) {
    var randomPhotoIndex = Math.floor(Math.random() * 100), // * flickrResponse.photos.photo.length instead? In case fewer than 100?
        ...
        picURL = 'https://flickr.com/' + randomPhoto.owner + '/' + randomPhoto.id + '/';         //compose (non-pretty) URL to Flickr image
    return picURL;
  });
};
