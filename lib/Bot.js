...
Bot.chartLyricsOptions = {
  ...
  stripRE:      /<.+>/g,
  processBody:  function(body) {
                  return body.match(Bot.chartLyricsOptions.goodRE)[0]
                        .replace(Bot.chartLyricsOptions.stripRE,'')
                        .trim ()
                        .split('\n');
                },
  validate:     function(error, response, body) {
...
Bot.ohhlaSongOptions  = {
  failRE:       /<title>.+Rap Geniuses.+<\/title>/i,
  targetRE:       /<pre>[\s\S]+<\/pre>/i,
  stripRE:      /<.+?>|^Typed by:.+$|^Artist:.+$|^Album:.+$|^Song:.+$/gm,
  processBody:  function(body) {
//                  console.log("body has <pre> at ", body.indexOf('<pre>'), '\t and </pre> at', body.indexOf('</pre>'));
//                  console.log("Search for goodRE gives:", body.search(Bot.ohhlaSongOptions.targetRE));
                  var lyrics;
                  try {
                    lyrics = body.match(Bot.ohhlaSongOptions.targetRE)[0];
                  }
                  catch(err) {
                    lyrics = body;
                  }
                  return lyrics.replace(Bot.ohhlaSongOptions.stripRE,'')
                        .trim()
                        .split('\n');
                },
  validate:     function(error, response, body) {
                  return (!error && response.statusCode === 200 && !(Bot.ohhlaSongOptions.failRE.test(body)) );
                },
  onError:      Bot.promiseDefaults.onError
};

Bot.ohhlaListOptions  = {
  failRE:       /<title>.+Rap Geniuses.+<\/title>/i,
  targetRE:     /".+?\.txt"/g,
  stripRE:      /"|http:\/\/ohhla.com\//g,
  processBody:  function(body) {
                  var relativePath = utils.randomFromArray(body.match(Bot.ohhlaListOptions.targetRE))
                                    .replace(Bot.ohhlaListOptions.stripRE,'')
                                    .trim ();
                  //console.log('Returning', 'http://ohhla.com/' + relativePath, 'from randomTopOhhla...')
                  return 'http://ohhla.com/' + relativePath;
                },
  validate:     function(error, response, body) {
                  return (!error && response.statusCode === 200 && !(Bot.ohhlaListOptions.failRE.test(body)) );
                },
  onError:      Bot.promiseDefaults.onError
};
...
Bot.ohhlaFavOptions = {
  ...
  failRE:       Bot.ohhlaListOptions.failRE,
  stripRE:      Bot.ohhlaListOptions.stripRE,
  processBody:  function(body) {
                  var relativePath = utils.randomFromArray(body.match(Bot.ohhlaFavOptions.targetRE))
                                    .replace(Bot.ohhlaFavOptions.stripRE,'')
                                    .trim ();
                  //console.log('Returning', 'http://ohhla.com/' + relativePath, 'from randomTopOhhla...')
                  return 'http://ohhla.com/' + relativePath;
                },
  validate:     Bot.ohhlaListOptions.validate,
  onError:      Bot.ohhlaListOptions.onError
};

Bot.cowboyIndexOptions = {
  targetRE:     /href="lyrics\/.+?"/gi,
  stripRE:      /"|href=/gi,
  processBody:  function(body) {
                  var artistLinks = body.match(Bot.cowboyIndexOptions.targetRE),
                      relativePath = utils.randomFromArray(artistLinks).replace(Bot.cowboyIndexOptions.stripRE,'');
...
Bot.cowboyArtistOptions = {
  targetRE:     /href="(?!http)[^\.]+?\/.+?\.html"/gi,
  stripRE:      /"|href=/gi,
  processBody:  function(body) {
                  var songLinks = body.match(Bot.cowboyArtistOptions.targetRE),
                      relativePath = utils.randomFromArray(songLinks).replace(Bot.cowboyArtistOptions.stripRE,'');
                  return "http://www.cowboylyrics.com/lyrics/" + relativePath;
                },
  validate:     Bot.promiseDefaults.validate,
  onError:      Bot.promiseDefaults.onError
};

Bot.cowboySongOptions = {
  targetRE:     /br>[\s\S]+?</gi,
  stripRE:      /br>|<|\n/gi,
  junkRE:       /^\[|^[\W\D]$|google_ad|Ringtone to your cell|Watch.+lesson$|DoubleClick|^Artist:|^Song:|^Album:|nbsp|href/gi,
  processBody:  function (body) {
                  var textArr = body.match(Bot.cowboySongOptions.targetRE),
                      fullLyric = [];
                    
                  _.each(textArr, function(el, ind, arr) {
                    var text = el.replace(Bot.cowboySongOptions.stripRE,'').trim();
                    if (text !== '' && !(Bot.cowboySongOptions.junkRE.test(text)) ) {
                      fullLyric.push(text);
                    }
                  });
...
Bot.prototype.makeOhhlaLyricsPromise = function(url, bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  return bot.makeRequestPromise(url, bot, Bot.ohhlaSongOptions);
};
...
Bot.prototype.makeRandomOhhlaSongPromise = function makeRandomOhhlaSongPromise(url, bot) {
  ...
  return bot.makeRequestPromise(url, bot, Bot.ohhlaListOptions);
};
...
Bot.prototype.makeRandomFavoriteOhhlaPromise = function makeRandomFavoriteOhhlaPromise(bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  return bot.makeRequestPromise('http://ohhla.com/favorite.html', bot, Bot.ohhlaFavOptions);
};
...
Bot.prototype.makeRandomCowboyArtistPromise = function(bot) {
  ...
  var artistIndex = "http://www.cowboylyrics.com/" + 
                    ...
                    ".html";
  return bot.makeRequestPromise(artistIndex, bot, Bot.cowboyIndexOptions);
};

Bot.prototype.makeRandomCowboySongPromise = function(artistPage, bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  return bot.makeRequestPromise(artistPage, bot, Bot.cowboyArtistOptions);
};

Bot.prototype.makeCowboyLyricPromise = function(songURL, bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  return bot.makeRequestPromise(songURL, bot, Bot.cowboySongOptions);
};
...
Bot.prototype.makeLyrpicTweet = function(bot) {
  ...
  var T = bot.T,
      ...
      if (bot.handle === 'rapcats') {
        lyricPromise  = bot.makeRandomFavoriteOhhlaPromise(bot).pipe(function (artistPage) {
                          return bot.makeRandomOhhlaSongPromise(artistPage, bot).pipe(function (songURL) {
                            return bot.makeOhhlaLyricsPromise(songURL, bot);
                          });
                        });
        /* Beneath will grab lyrics from a random "Top 30" track on ohhla 
        lyricPromise =  bot.makeRandomOhhlaSongPromise(null, bot).pipe(function(songURL) {
                          return bot.makeOhhlaLyricsPromise(songURL, bot);
                        });
        */
      }
      else {
        lyricPromise =  bot.makeRandomCowboyArtistPromise(bot).pipe(function (artistURL) {
          return bot.makeRandomCowboySongPromise(artistURL, bot).pipe(function (songURL) {
            return bot.makeCowboyLyricPromise(songURL, bot);
          });
        });
        /*
        artistTitlePair = bot.getArtistTitlePair();
        lyricPromise = bot.makeChartLyricsPromise(utils.makeChartLyricsURL(artistTitlePair), bot);
        */
      }
