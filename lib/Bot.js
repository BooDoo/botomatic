...
Bot.prototype.tweetMashFilter = function tweetMashFilter(tweetTexts, targetPool, bot) {
  ...
  var since_id = targetPool.since_id;
  bot = utils.setArgDefault(bot, this, Bot);
...
Bot.prototype.populateTweetMashSources = function populateTweetMashSources(bot) {
  ...
  */
  ...
  firstPromise.pipe(function (tweetTexts) {
    console.log("Filtering", tweetTexts.length, "firsts...");
    bot.firsts = tweetMashFilter(tweetTexts, bot.firsts, bot);
  });
  
  secondPromise.pipe(function (tweetTexts) {
    console.log("Filtering", tweetTexts.length, "seconds...");
    bot.seconds = tweetMashFilter(tweetTexts, bot.seconds, bot);
  });
  */
};
