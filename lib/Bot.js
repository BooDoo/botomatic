...
Bot.prototype.getLyricPromise = function getLyricPromise(url, bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  var lDeferred = _.Deferred(),
      lyricPromise = lDeferred.promise(),
      fullLyric = [],
      emptyRE = /<Lyric \/>/, //check for blank Lyric section of result
      lyrRE = /<Lyric>([\s\S]+?)<\/Lyric>/; //match the Lyrics content
  
  request({
    url: url
  }, function(error, response, body) {
    if (!error && response.statusCode === 200 && !emptyRE.test(body)) {
      //Got lyrics - strip out HTML tags and send an array split by newline
      fullLyric = body.match(lyrRE)[0].replace(/<.+>/,'').trim().split('\n');
      console.log("fullLyric.length: ", fullLyric.length);
      lDeferred.resolve(fullLyric); 
    }
    else {
      console.log("Rejecting lyricPromise");
      lDeferred.reject(error || "Status code: " + response.statusCode + '\t\t(maybe no Lyric found?)'); 
    }
  });
...
Bot.prototype.getFlickrPromise = function getFlickrPromise(url, bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  var fDeferred = _.Deferred(),
      flickrPromise = fDeferred.promise();
  ...
  request({
    ...
    if (!error && response.statusCode === 200) {
      if (typeof body !== 'object') {
        console.log("Parsing Flickr response from string to object");
        body = JSON.parse(body);
      }
      fDeferred.resolve(body); 
    }
    else {
      console.log("Rejecting flickrPromise");
      fDeferred.reject(error || "No error, but statusCode not 200"); 
    }
  });
...
Bot.prototype.makeLyrpicTweet = function(bot) {
  ...
  var T = bot.T,
      ...
      lyricPromise = bot.getLyricPromise(utils.makeChartLyricsURL(artistAndTitle), bot),
