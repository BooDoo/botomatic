...
Bot.promiseDefaults = 
{
  ...
  validate:     function(error, response, body) {
                  return (!error && response.statusCode === 200 && !Bot.promiseDefaults.failRE.test(body));
                },
  ...
  onError:    function(error, response, body) {
                return (error || "Status code: " + response.statusCode + '\t\t(failed validation?)'); 
              }
};
...
Bot.prototype.getLyricPromise = function getLyricPromise(url, promiseOptions, bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  var lDeferred = _.Deferred(),
      lyricPromise = lDeferred.promise(),
      options = utils.setArgDefault(promiseOptions, bot.getPromiseDefault()),
      fullLyric = [];
  ...
  request({
    ...
    if (options.validate()) {
      fullLyric = options.processBody(body);
      lDeferred.resolve(fullLyric);
    }
    else {
      lDeferred.reject(options.onError(error, response)); 
    }
  });
