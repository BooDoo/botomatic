...
Bot.prototype.populateLyricryptic = function populateLyricryptic(bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  return bot.twitterSearch('from:lyricryptic',bot).pipe(function (lyricSegments) {
    _.each(lyricSegments, function(el, ind, arr) {
      bot.lyricrypticPool[el] = el;
    });
  });
};

Bot.prototype.twitterSearch = function twitterSearch(query, bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  var searchURL = "http://search.twitter.com/search.json?callback=?&rpp=100&q='" + encodeURIComponent(query) + "'&result_type=recent"; //TODO: Fill this in! 
  return bot.makeRequestPromise(searchURL, 'twitterSearch');
};
...
Bot.prototype.ohhlaRandomTop30Lyric = function ohhlaRandomTop30Lyric(bot) {
  ...
  return  bot.makeRandomOhhlaSongPromise(null, bot).pipe(function(songURL) {
            return bot.makeOhhlaLyricsPromise(songURL, bot).pipe(function (fullLyric) {
...
Bot.prototype.ohhlaRandomFaveLyric = function ohhlaRandomFaveLyric(bot) {
  ...
  return  bot.makeRandomFavoriteOhhlaPromise(bot).pipe(function (artistPage) {
            return bot.makeRandomOhhlaSongPromise(artistPage, bot).pipe(function (songURL) {
              return bot.makeOhhlaLyricPromise(songURL, bot).pipe(function (fullLyric) {
                return utils.randomFromArray(Word.findRhymes(fullLyric));
              });
            });
          });
...
Bot.prototype.cowboyRandomLyric = function cowboyRandomLyric(bot) {
  ...
  return  bot.makeRandomCowboyArtistPromise(bot).pipe(function (artistURL) {
            return bot.makeRandomCowboySongPromise(artistURL, bot).pipe(function (songURL) {
              return bot.makeCowboyLyricPromise(songURL, bot).pipe(function (fullLyric) {
                return utils.randomFromArray(Word.findRhymes(fullLyric));
              });
            });
          });
...
Bot.prototype.azRandomLyric = function azRandomLyric(bot) {
  ...
  return  bot.makeRandomAzArtistPromise(bot).pipe(function (artistURL) {
            return bot.makeRandomAzSongPromise(artistURL, bot).pipe(function (songURL) {
              return bot.makeAzLyricPromise(songURL, bot).pipe(function (fullLyric) {
                return utils.randomFromArray(Word.findRhymes(fullLyric));
              });
            });
          });
...
Bot.prototype.flickrRandomPhotoURL = function flickrRandomPhotoURL(bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  return bot.makeFlickrPromise(bot.getFlickrURL(bot), bot).pipe(function(flickrResponse) {
...
Bot.prototype.makeLyrpicTweet = function(bot) {
  ...
  var T = bot.T,
      ...
      if (bot.lyricType === 'rap') {
        ...
        //make promise chain to get a random @lyricryptic tweet with length < 117 #thankUbasedBrendan
      }
      else {
  ...
  .then(function(lyricSegment, photoURL) {
    ...
    //lyricPromise should contain a tweetable (preferably rhyming) extract from lyrics
    //rhymes = Word.findRhymes(fullLyric);
    tweetContent += lyricSegment;
    ...
    //photoURL should be a random (non-pretty) Flickr photo URL returned from piped promise chain.
    //append the Flickr URL to our tweet and output to log for reference
    tweetContent += ' ' + photoURL;
