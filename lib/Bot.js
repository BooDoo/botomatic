...
function Bot (botConfig) {
  ...
  if (this.type === 'lyrpictweet') {
    ...
    this.intervalId = setInterval(this.tweetFromQueue, this.interval, this);
      ...
      this.firstCriterion = this.criteria[0]; //"http://search.twitter.com/search.json?callback=?&rpp=100&q='" + encodeURIComponent(this.criteria[0]) + "'&result_type=recent";
      this.secondCriterion = this.criteria[1]; //"http://search.twitter.com/search.json?callback=?&rpp=100&q='" + encodeURIComponent(this.criteria[1])  + "'&result_type=recent";
      this.firsts = [];
      this.seconds = [];
      this.shortSeconds = [];
      this.pre = "";
...
Bot.prototype.tweetMashFilter = function tweetMashFilter(tweetTexts, targetPool, bot) {
  ...
  var passedFilter =  _(tweetTexts).filter(function (text) {
...
Bot.prototype.tweetMashAltFilter = function tweetMashAltFilter(tweetTexts, targetPool, bot) {
  ...
  var passedAltFilter = _(tweetTexts).filter( function (text) {
...
Bot.prototype.populateTweetMashSources = function makeTweetMash(bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  var firstPromise = utils.twitterSearch(bot.criteria[0], bot.T, "twitterTextOnly", bot.firsts),
      secondPromise = utils.twitterSearch(bot.criteria[1], bot.T, "twitterTextOnly", bot.seconds);
  // get the swag tweets, then the latour tweets, then mash together. 
  // TODO: replace with twitterSearch() promise
  
  //Use when().then?
  /*
  _.when(firstPromise)
  .then(tweetMashFilter(firstPromise, bot.firsts, bot),
  console.log
  );
  
  _.when(secondPromise)
  .then(tweetMashFilter(secondPromise, bot.seconds, bot),
  console.log
  );
  */
  ...
  //Or use pipe instead?
  /*
  firstPromise.pipe(function (tweetTexts) {
    tweetMashFilter(firstPromise, bot.firsts, bot);
  });
  
  secondPromise.pipe(function (tweetTexts) {
    tweetMashFilter(secondPromise, bot.seconds, bot);
  });
  */
};
...
Bot.prototype.composeTweetMash = function composeTweetMash (bot) {
  ...
  if (secondfirst) {
    text = utils.randomFromArray(seconds, false, null, true); //seconds[Math.floor(Math.random() * seconds.length)];
  }
  else {
    text = utils.randomFRomArray(firsts, false, null, true);  //firsts[Math.floor(Math.random() * firsts.length)];
  }
  ...
  if (secondfirst) {
    ...
    text = utils.randomFromArray(allSeconds, false, null, true); //TODO: Doesn't strip from origin since this is a composited array!
  }
