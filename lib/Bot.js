...
function Bot (botConfig, botStore) {
  ...
  if (this.type === 'lyrpictweet') {
    ...
    this.populateSnowcloneWords(this); //Initial population of the random word pools.
    this.searchIntervalId = setInterval(this.populateSnowcloneWords, this.searchInterval, this);
...
Bot.prototype.populateSnowcloneWords = function(bot) {
  bot = utils.setArgDefault(bot, this, Bot);
  ...
  for (wh in words) {
    ...
    Word.randomWords(api_key, reqParams)
        .pipe(function(words) {
          bot.moveWordsToPool(words, wh, bot);
        });
  }
};
