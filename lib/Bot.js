...
function Bot (botConfig) {
  ...
  if (this.type === 'lyrpictweet') {
    ...
    this.firsts = [];
    this.seconds = [];
    this.composeTwoSources(this.prioritySource, this.preSource, this); //Initial population of source pools
    ...
    this.intervalId = setInterval(this.composeTwoSources, this.interval, this, this.prioritySource, this.preSource);
  }
    ...
    _.each(arguments, function (docBody, index) {
      ...
      _.each(descBoxes, function (el, i) {
        ...
        _.each(splitEl, function (split, index) {
          split = split.trim();
          if (/\w/.test(split) && split.length <= maxLength) {
...
Bot.prototype.populateLatourAndOrderSources = function populateLatourAndOrderSources(bot) {
  ...
  if (_.isEmpty(bot.firsts)) {
    ...
    var replaceParams = [/((?:\w\.){2,}|Sgt\.|Det\.|Sen\.|Ph\.(?:D\.?)?|Dr\.|st\.)/ig, /\./g, ""];
    bot.populateIMDBDescriptions("tt0098844", 20, replaceParams, bot.firsts);
    //bot.populateIMDBDescriptions("tt0203259", 14, replaceParams, bot.firsts);
...
Bot.prototype.composeTwoSources = function composeTwoSources(prioritySource, preSource, bot) {
  ...
  var T = bot.T,
      ...
      //seconds = bot.seconds,//Or just assign primaries/others staright off?
      primaries, primary,
...
//HACKY! Should be abstracted out.
//Abstracts to: composeTwoSources(1, 0, bot) (pending testing)
