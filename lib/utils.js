...
module.exports = {
  ...
  postTweet: function postTweet(T, tweet) {
    ...
    if (process.env['NODE_ENV'] === 'production') {
      T.post('statuses/update', tweet, function (err, reply) {
        if (err) {
          util.error('ERROR tweeting: ' + util.inspect(err));
        }
        else {
          util.debug('Successfully tweeted: ' + tweet.status);
        }
      });
    }
    else {
      util.log('Would tweet: ' + tweet.status);
    }
  },
  ...
  searchTweets: function searchTweets(T, criteria) {
    ...
    //util.debug('Searching with ' + util.inspect(criteria));
