var utils         = require(__dirname + '/utils.js'),
    PromiseConfig = require(__dirname + '/PromiseConfig.js'),
...
module.exports = {
  makeTwitterSearchPromise: function makeTwitterSearchPromise(criteria, T, options, tweetPool) {
    ...
    // util.debug("tweetPool.since_id within makeTwitterSearchPromise: " + tweetPool.since_id);
    criteria.since_id = criteria.since_id || tweetPool.since_id || 0;

    T.get('search/tweets', criteria, function(err, reply) {
      // util.debug('Got response for ' + util.inspect(criteria));
      if(err) {
        ...
        //util.debug(reply.statuses.length + " tweets returned, since_id at: " + tweetPool.since_id + " for " + criteria.q);
  ...
  makeRequestPromise: function makeRequestPromise(url, promiseOptions) {
    ...
    request({
      ...
      if (options.validate.call(options, error, response, body)) {
        //util.debug("Got response for " + url);
