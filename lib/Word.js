...
var Word = function Word (literal, cmu) {
  try {
    ...
    util.error("No literal value given for new word!");
    return null;
  }
...
Word.prototype.countSyllables = function(word) {
  ...
  if (word.syllables) { //Already counted? Return stored value.
    return word.syllables;
  }

  word.syllables = 0;
  //Increment syllable count for each phoneme starting A/E/I/O/U
  _.each(word.phonemes,
    function (phoneme) {
      if (phoneme.match(/^[AEIOU]/i))
        {word.syllables += 1;}
    }
  )
...
Word.phrasePres = function(word, params, wordnikKey) {
  ...
  wnPromise.then(
    ...
    function(e) {util.error("Phrasal prefixes for " + word.literal + " not retrieved.");}
  );
...
Word.randomWords = function(params, wordnikKey) {
  ...
  //util.debug("randomWords endpoint: " + wnEndpoint);
  return Promises.makeRandomWordsPromise(wnEndpoint);
};

Word.descriptors = function(word, params, wordnikKey) {
  if (!(word instanceof Word)) {word = new Word(word);} //as Word()
  if (word._descriptors) {return when.resolve(word._descriptors);}    //already cached
  wordnikKey = wordnikKey || Word.wordnikKey;
  params = params || {};
  params.api_key = params.api_key || wordnikKey;
  if (!params.api_key) {return when.reject("Wordnik API key required");}

  //util.debug("Getting descriptors for: " + word.literal);
