...
Word.phrasePres = function(word, params, wordnikKey) {
  ...
  params = params || {"limit": 200, "wlmi": 8};
  ...
  wnPromise.then(
    function(phrasePres) {word._phrasePres = phrasePres; console.log("Stored phrasal prefixes for",word.literal);},
...
Word.partOfSpeech = function(word, params, wordnikKey) {
  if (!(word instanceof Word)) {word = new Word(word);} //as Word()
  if (word._partOfSpeech) {return word._partOfSpeech;}  //already cached
  ...
  wnPromise.then(
    function(partOfSpeech) {word._partOfSpeech = partOfSpeech; console.log("Stored parts of speech for",word.literal);},
    function(e) {console.error("Phrasal prefixes for",word.literal,"not retrieved.");}
  );
...
Word.isDescriptor = function(word, wordnikKey) {
  if (!(word instanceof Word)) {word = new Word(word);} //as Word()
  if (word._isDescriptor) {return word._isDescriptor;}  //already cached
  ...
  .then(
    function(pos) {word._isDescriptor = descRE.test(pos); descDeferred.resolve(word._isDescriptor);},
